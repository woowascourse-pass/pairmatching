...
### 페어 매칭
- **1. `camp.nextstep.edu.missionutils.Randoms`의 shuffle 메서드의 결과로 순서대로 페어**

### 정상 입력

- 기능 선택은 1,2,3,Q 로만 입력한다.
- 정보 입력시 과정, 레벨, 미션의 순으로 입력하며 출력된 정보에 존재하는 문자열을 쉼표(,)를 구분하로 하여 입력한다.
- 재매칭 여부는 네/아니오 로만 입력한다.

### 예외 처리

잘못된 값을 입력할 경우 `IllegalArgumentException`을 발생시키고, "[ERROR]"로 시작하는 에러 메시지를 출력 후 그부분 부부터 입력을 다시 받는다.

#### 잘못된 입력

- 기능 선택시 1,2,3,Q 이외의 문자를 입력하는 경우
- 정보 입력시 존재하지 않는 정보를 입력하는 경우
- 정보 입력시 구분자로 쉼표(,)를 사용하지 않는 경우
- 재매칭 여부 입력 시 네/아니오 이외의 문자를 입력하는 경우
